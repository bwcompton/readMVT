...
# An example of using readMVT to display vector data in Leaflet under Shiny

## Introduction

Although Leaflet can display very large image or raster data seamlessly with little effort, out of the box, it is far more limited in displaying vector data. The built-in methods require that your datasets be small enough to fit in the client's browser memory, and datasets of more than a very few thousand points are prohibitively slow to load.

One nice approach to displaying vector data is via [Mapbox Vector Tiles](https://docs.mapbox.com/data/tilesets/guides/vector-tiles-introduction/) (MVT) served by a [GeoServer](https://geoserver.org/). Tiling is done transparently on the GeoServer, so data prep doesn't require a lot of effort. The readMVT package facilitates finding, loading, and displaying MVTs in Leaflet. It is intended to be run under Shiny, but can be used in simpler situations too.
...
To speed things up considerably, data are cached at two levels. First, we use the package *memoise* to cache tiles as they are fetched from the GeoServer. If there are multiple client browsers under the same R session, this cache is shared, so the same tile is never fetched from the GeoServer twice by the same R session. Second, Leaflet internally caches data drawn by the add\* functions (here, addPolylines and addCircleMarkers). We track (by user, not session) which tiles have been drawn so they don't need to be redrawn for a user.

If you want to try running this example, we've provided it in one big chunk at the bottom, all ready to be copied and pasted .
...
These three variables have to do with zoom levels. MVTs are simplified at lower zoom levels. As we've chosen not to display vector data at lower zooms, we pick a fixed zoom to simplify caching and reduce fetching redundant data. At zoom level 14, data are not simplified, so that seems like a good choice.
...
This line sets up the memoise cache for fetched MVTs. You can omit this, but that seems like a bad idea for any real-world application.

```         
read.tile.C <<- memoise(read.tile)                          # set up read.tile to cache vector tiles (global to share among users)
```

Finally, we can use functions from readMVT. read.XML fetches the GeoServer's "capabilities" file, an XML with information on all data on the GeoServer. Then, for each feature we will fetch, layer.info extracts an MVT object from the XML.
...
13   12      1505      1532      1210      1252
14   13      3011      3064      2421      2505
15   14      6023      6129      4843      5011
  ...
...
Here's a simple Shiny user interface. We're making a sidebar on the left to display the zoom level.
...
And finally, the guts of the example.
...
Everything interesting happens inside an *observe*. First, we display the zoom level in the left sidebar, and "TRIGGERED!" if the zoom level is above the trigger. You should expect to see streams and crossings with TRIGGERED is shown.
...
If we're below the trigger, we're done--the basemap will be displayed, but none of the vector data. If we're zoomed above (or equal to) the trigger, we'll read the vector tiles and render them. We use leafletProxy to modify the current display so there's no unnecessary (and ugly) redrawing.
...
We do the same thing for culverts. Note that we could pull culverts into a separate if clause with a different trigger if we wanted to.
...
The call to groupOptions displays all previously-rendered vector data at the selected zoom levels. Without this, zooming out would show streams and culverts for tiles we've visited, but not other visible tiles. By hiding the vector data, they smoothly vanish as we zoom out. The Leaflet map is returned and rendered as the observe finishes.
...
If you're using RStudio, you should be able to install the named packages, copy this code, paste it into a script window, and click "Run App."
